...
EOF
```
(9=더미 데이터, 2=주문메뉴, 1=접수 → 시료 목록에서 1번(S-001) 선택,
...수량/고객명 순서로 입력(수량이 먼저다 — 수량 검증에 실패해도 고객명을
버리지 않기 위해서)..., 2=승인 → 방금 만든 주문은 RESERVED 목록의 2번째
(1번째는 더미 데이터), 0=주문메뉴에서 뒤로가기, 4=출고 처리 → 방금 승인한
주문은 CONFIRMED 목록의 2번째, 0=종료. **접수 시 시료 ID를 직접 타이핑하지
않고 번호로 선택한다** — 존재하지 않는 시료로 주문을 만들 수 없다.)
...
- 주문 접수("주문" → "접수")는 시료 ID를 직접 타이핑하는 게 아니라,
  등록된 시료 목록에서 번호로 선택하는 방식이다. 등록된 시료가 없으면
  "등록된 시료가 없습니다"만 출력하고 주문을 만들지 않는다 (크래시 없음).
